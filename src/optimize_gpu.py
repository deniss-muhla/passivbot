...
from backtest_cuda import run as run_backtest_cuda
...
if CUDA_AVAILABLE:
    ...
    logging.info(f"CUDA is available. Using GPU acceleration with device: {torch.cuda.get_device_name(0)}")
    logging.info(f"CUDA Version: {torch.version.cuda}")
else:
    gpu_error = ""
    if not torch.version.cuda:
        gpu_error = "PyTorch was not built with CUDA support"
    elif not torch.cuda.is_available():
        if sys.platform == "win32":
            gpu_error = "NVIDIA GPU driver might not be installed or is not compatible"
        else:
            gpu_error = "NVIDIA drivers might not be installed or GPU is not accessible"
    logging.warning(f"CUDA is not available: {gpu_error}. Using CPU for computations")
...
class Evaluator:
    def __init__(self, shared_memory_file, hlcvs_shape, hlcvs_dtype, config, mss, results_queue):
        ...
        if self.cuda_available:
            logging.info(f"GPU Memory Usage before initialization:")
            logging.info(f"  Allocated: {torch.cuda.memory_allocated(0) / 1024**2:.2f} MB")
            logging.info(f"  Reserved:  {torch.cuda.memory_reserved(0) / 1024**2:.2f} MB")
            torch.backends.cudnn.benchmark = True
        ...
        if self.cuda_available:
            try:
                hlcvs_copy = np.array(self.shared_hlcvs_np, dtype=np.float64)
                self.shared_hlcvs_cuda = torch.from_numpy(hlcvs_copy).cuda()
                logging.info(f"Data successfully transferred to CUDA device with dtype: {self.shared_hlcvs_cuda.dtype}")
                logging.info(f"Original numpy dtype: {self.shared_hlcvs_np.dtype}")
                logging.info(f"GPU Memory Usage after data transfer:")
                logging.info(f"  Allocated: {torch.cuda.memory_allocated(0) / 1024**2:.2f} MB")
                logging.info(f"  Reserved:  {torch.cuda.memory_reserved(0) / 1024**2:.2f} MB")
            except RuntimeError as e:
                logging.error(f"Failed to transfer data to GPU: {str(e)}")
                self.cuda_available = False
                logging.warning("Falling back to CPU computation")

        self.config = config
        _, self.exchange_params, self.backtest_params = prep_backtest_args(config, mss)
        logging.info(f"Evaluator initialization complete. Using {'GPU' if self.cuda_available else 'CPU'} for computations")
        self.results_queue = results_queue

    def evaluate(self, individual):
        try:
            config = individual_to_config(individual, self.config)
            bot_params, exchange_params, backtest_params = prep_backtest_args(
            ...
            if self.cuda_available:
                try:
                    with torch.cuda.device(0):
                        ...
                        fills, equities, analysis = run_backtest_cuda(
                            self.shared_hlcvs_cuda,
                            bot_params,
                            exchange_params,
                            backtest_params
                        )
                except Exception as e:
                    logging.error(f"CUDA backtest failed, falling back to CPU: {str(e)}")
                    self.cuda_available = False
    ...
    def __del__(self):
        if hasattr(self, 'shared_hlcvs_cuda'):
            del self.shared_hlcvs_cuda
            torch.cuda.empty_cache()
...
async def main():
    ...
    try:
        ...
        if CUDA_AVAILABLE:
            pool = mp.Pool(processes=config["optimize"]["n_cpus"])
            logging.info("Using torch.multiprocessing pool for CUDA support")
        else:
            pool = multiprocessing.Pool(processes=config["optimize"]["n_cpus"])
            logging.info("Using standard multiprocessing pool")
...
if __name__ == "__main__":
    if torch.cuda.is_available():
        try:
            ...
            logging.info("Multiprocessing start method set to 'spawn' for CUDA support")
        except RuntimeError:
            if mp.get_start_method() != 'spawn':
                logging.error("Failed to set multiprocessing start method to 'spawn'. CUDA may not work correctly.")
            else:
